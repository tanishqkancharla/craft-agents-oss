...
export class BrowserPaneManager implements IBrowserPaneManager {
  ...
  private stateChangeCallback: ((info: BrowserInstanceInfo) => void) | null =
    null;
  private removedCallback: ((id: string) => void) | null = null;
  private instanceDestroyedCallback: ((instanceId: string) => void) | null = null;
  ...
  private popupWindowsByParentInstanceId = new Map<
    string,
    Set<BrowserWindow>
  >();
  private popupParentByWebContentsId = new Map<number, string>();
  private windowManager: WindowManager | null = null;
  private sessionPathResolver: ((sessionId: string) => string | null) | null =
    null;
  ...
  onInstanceDestroyed(callback: (instanceId: string) => void): void {
    this.instanceDestroyedCallback = callback;
  }
  ...
  createInstance(id?: string, options?: CreateBrowserInstanceOptions): string {
    ...
    const ownerSessionId =
      ownerType === "session" ? (options?.ownerSessionId ?? null) : null;

    if (this.instances.has(instanceId)) {
      mainLog.warn(
        `[browser-pane] Instance already exists, reusing: ${instanceId}`,
      );
      return instanceId;
    }
    ...
    const supportsMultiView =
      typeof window.addBrowserView === "function" &&
      typeof window.setTopBrowserView === "function";
    if (!supportsMultiView) {
      throw new Error(
        "[browser-pane] Native overlay requires BrowserWindow.addBrowserView + setTopBrowserView",
      );
    }
    ...
    const toolbarWcWithBg =
      toolbarView.webContents as typeof toolbarView.webContents & {
    ...
    const overlayWcWithBg =
      nativeOverlayView.webContents as typeof nativeOverlayView.webContents & {
        setBackgroundColor?: (color: string) => void;
      };
    ...
    void this.loadEmptyStatePage(instance).catch((error) => {
      mainLog.warn(
        `[browser-pane] empty-state load failed id=${instance.id}: ${error instanceof Error ? error.message : String(error)}`,
      );
  ...
  destroyInstance(id: string): void {
    ...
    if (!instance) {
      mainLog.info(
        `[browser-pane] destroy requested for missing instance id=${id}`,
      );
      return;
    }
    ...
    this.prepareForFinalDestroy(instance);
    ...
    runCleanup("closePopupsForParent", () =>
      this.closePopupsForParent(instance.id, "parent_destroy"),
    );
    runCleanup("applyAgentControlLock", () =>
      this.applyAgentControlLock(instance, false),
    );
    runCleanup("updateNativeOverlayState", () =>
    ...
    try {
      ...
      mainLog.warn(
        `[browser-pane] destroy failed id=${id} error=${error instanceof Error ? error.message : String(error)}`,
      );
    } finally {
      // Finalize synchronously in case closed does not fire (or fires later).
      this.finalizeDestroyedInstance(instance, "destroy");
      mainLog.info(
        `[browser-pane] destroy completed id=${id} removed=${!this.instances.has(id)}`,
      );
    }
  }
  ...
  private cleanupDestroyedInstance(
    ...
    mainLog.info(
      `[browser-pane] cleaned up stale instance ${instance.id}: ${reason}`,
    );
  }
  ...
  async handleEmptyStateLaunchFromRenderer(
    ...
    if (!instance) {
      mainLog.warn(
        `[browser-pane] empty-state launch ignored: sender not mapped senderWebContentsId=${senderWebContentsId}`,
      );
    ...
    if (!route) {
      mainLog.warn(
        `[browser-pane] empty-state launch missing route id=${instance.id}`,
      );
      return { ok: false, handled: false, reason: "missing_route" };
    }

    const token = payload.token ?? null;
    const handled = await this.triggerEmptyStateRouteLaunch(
      instance,
      route,
      token,
      "ipc",
  ...
  private resolveLaunchWorkspaceId(): string | null {
    ...
    if (focusedWindow) {
      const focusedWorkspaceId = this.windowManager.getWorkspaceForWindow(
  ...
  private async triggerEmptyStateRouteLaunch(
    ...
    if (dedupeToken && instance.lastLaunchToken === dedupeToken) {
      mainLog.info(
        `[browser-pane] ignoring duplicate empty-state launch id=${instance.id} source=${source} token=${dedupeToken}`,
      );
      return false;
    }
    ...
    mainLog.info(
      `[browser-pane] handling empty-state launch id=${instance.id} source=${source} route=${route} deepLink=${deepLink}`,
    );
  ...
  async navigate(
    ...
    if (!hasScheme && !isAbout) {
      const looksLikeHost =
        /^(localhost|\d{1,3}(?:\.\d{1,3}){3}|[\w-]+(?:\.[\w-]+)+)(?::\d+)?(?:\/|$)/i.test(
          normalizedUrl,
        );
      if (looksLikeHost) {
      ...
      const timeout = new Promise<never>((_, reject) => {
        timeoutHandle = setTimeout(
          () =>
            reject(
              new Error(
                `Navigation to "${normalizedUrl}" timed out after ${timeoutMs / 1000}s`,
              ),
            ),
          timeoutMs,
        );
  ...
  focus(id: string): void {
    ...
    if (!instance.toolbarReady) {
      ...
      mainLog.info(
        `[browser-pane] focus deferred until ready id=${instance.id} token=${token}`,
      );
      return;
    }
      ...
      if (waitFor === "navigation") {
        ...
        await new Promise<void>((resolve, reject) => {
          ...
          const cleanup = () => {
            ...
            instance.pageView.webContents.removeListener(
  ...
  private suspendOverlayForCapture(instance: BrowserInstance): boolean {
    const shouldSuspend =
      !!instance.agentControl?.active && instance.nativeOverlayReady;
    ...
    try {
        ...
        return {
          ...
          metadata: options?.includeMetadata
            ? {
                mode: "raw",
                warnings:
                  captured.warnings.length > 0 ? captured.warnings : undefined,
              }
            : undefined,
        };
      }
      ...
      if (annotate) {
        try {
          ...
          refs = snapshot.nodes
            .map((node) => node.ref)
            .slice(0, MAX_ANNOTATED_REFS);
          if (snapshot.nodes.length > MAX_ANNOTATED_REFS) {
            warnings.push(
              `Annotation capped at ${MAX_ANNOTATED_REFS} of ${snapshot.nodes.length} elements`,
            );
          }
        } catch (error) {
          warnings.push(
            `Accessibility snapshot for annotation failed: ${error instanceof Error ? error.message : String(error)}`,
          );
          refs = [];
        }
      }
      ...
      try {
        ...
        warnings.push(
          `Annotation overlay failed: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
  ...
  async screenshotRegion(
    ...
    const hasCoords = [target.x, target.y, target.width, target.height].every(
      (v) => typeof v === "number",
    );
    ...
    if (modeCount === 0) {
      throw new Error(
        "Region screenshot requires either coordinates, ref, or selector",
      );
    }
    if (modeCount > 1) {
      throw new Error(
        "Region screenshot target is ambiguous. Provide only one of coordinates, ref, or selector",
      );
    }
    ...
    try {
      ...
      if (hasRef) {
        const geometry = await instance.cdp.getElementGeometry(
          String(target.ref),
        );
        box = { ...geometry.box };
      } else if (hasSelector) {
        const geometry = await instance.cdp.getElementGeometryBySelector(
      ...
      if (
        ...
        throw new Error(
          "Resolved screenshot region is outside the current viewport",
        );
      }
  ...
  private async capturePageWithRecovery(
    ...
    for (
      let attempt = 1;
      attempt <= SCREENSHOT_HIDDEN_CAPTURE_ATTEMPTS;
      attempt += 1
    ) {
      ...
      if (result) {
        if (attempt > 1) {
          warnings.push(
            `Capture recovered after ${attempt} hidden attempt${attempt === 1 ? "" : "s"}.`,
          );
        }
        return {
    ...
    if (!window.isDestroyed()) {
      try {
        ...
        let rescueResult: { buffer: Buffer; format: "png" | "jpeg" } | null =
          null;
        try {
        ...
        if (rescueResult) {
          if (rescueUsed) {
            warnings.push(
              "Capture required temporary inactive reveal for rendering; browser visibility was restored immediately.",
            );
          }
          return {
    ...
    throw new Error(
      `Failed to capture ${options.errorPrefix}: empty image buffer`,
    );
  }
  ...
  private isDisplaySurfaceUnavailableError(error: unknown): boolean {
    ...
    return error.message
      .toLowerCase()
      .includes("current display surface not available for capture");
  }
  ...
  private async capturePageImage(
    ...
    let image = options.rect
      ? await instance.pageView.webContents.capturePage(
          options.rect,
          captureOpts,
        )
    ...
    const encoded =
      fmt === "jpeg" ? image.toJPEG(options.jpegQuality ?? 80) : image.toPNG();
  ...
  async waitFor(id: string, args: BrowserWaitArgs): Promise<BrowserWaitResult> {
    ...
    if (args.kind === "network-idle") {
      ...
      return until(async () => {
        ...
        const last =
          this.lastNetworkActivityByWebContentsId.get(wcId) ?? started;
  ...
  async sendKey(id: string, args: BrowserKeyArgs): Promise<void> {
    ...
    const modifiers = (args.modifiers ?? []) as Array<
  ...
  async getDownloads(
    ...
    if (action === "wait") {
      const timeoutMs = Math.max(
        100,
        Number(options?.timeoutMs ?? DEFAULT_WAIT_TIMEOUT_MS),
      );
      const started = Date.now();
      while (Date.now() - started <= timeoutMs) {
        const hasTerminal = instance.downloads.some(
          (d) =>
            d.state === "completed" ||
            d.state === "interrupted" ||
            d.state === "cancelled",
  ...
  windowResize(
    ...
    instance.window.setContentSize(
      ...
      requestedViewportHeight + TOOLBAR_HEIGHT,
    );

    this.layoutAllViews(instance);

    // Return effective viewport dimensions after OS/window min-size constraints are applied.
    const [appliedContentWidth, appliedContentHeight] =
      instance.window.getContentSize();
    return {
  ...
  async detectSecurityChallenge(
    id: string,
  ): Promise<{ detected: boolean; provider: string; signals: string[] }> {
    const instance = this.instances.get(id);
    ...
    try {
      const domSignals = (await instance.pageView.webContents
        .executeJavaScript(`(() => {
        const signals = [];
    ...
    try {
      ...
      if (snapshot.nodes.length > 0 && actionableCount <= 2) {
        signals.push(
          `ax:near-empty(${actionableCount}/${snapshot.nodes.length})`,
        );
      }
    } catch {
    ...
    if (detected) {
      mainLog.info(
        `[browser-pane] security challenge detected id=${id} provider=${provider} signals=[${signals.join(", ")}]`,
      );
    }
  ...
  async getPageViewTargetId(id: string): Promise<string | null> {
    ...
    if (targetInfo.type !== "page") {
      mainLog.warn(
        `[browser-pane] Unexpected CDP target type for ${id}: ${targetInfo.type} url=${targetInfo.url} title=${targetInfo.title}`,
      );
    }
  ...
  setLibrettoAttachment(
    id: string,
    attachment: { librettoSession: string; pageTargetId: string | null },
  ): void {
    const instance = this.requireAliveInstance(id);
  ...
  getLibrettoAttachment(
    id: string,
  ): { librettoSession: string; pageTargetId: string | null } | null {
    const instance = this.instances.get(id);
    if (!instance || instance.window.isDestroyed() || !instance.librettoSession)
      return null;
    return {
    ...
    for (const instance of this.instances.values()) {
      if (instance.boundSessionId === sessionId) {
        if (instance.librettoSession) {
          mainLog.info(
            `[browser-pane] Preserved binding for attached instance ${instance.id} session=${sessionId} librettoSession=${instance.librettoSession}`,
          );
          continue;
        }
  ...
  createForSession(sessionId: string, options?: { show?: boolean }): string {
    ...
    if (reusable) {
      ...
      mainLog.info(
        `[browser-pane] Reused unbound instance ${reusable.id} for session ${sessionId}`,
      );
      return reusable.id;
    }
  ...
  private getAgentControlLabel(
    agentControl:
      | Pick<AgentControlState, "displayName" | "intent">
      | null
      | undefined,
  ): string {
  ...
  private getResolvedAccentColor(): string {
    ...
    const accent = isDark
      ? (userTheme?.dark?.accent ??
        userTheme?.accent ??
        DEFAULT_THEME.dark!.accent!)
      : (userTheme?.accent ?? DEFAULT_THEME.accent!);
    return accent;
  }

  private async loadNativeOverlayPage(
    instance: BrowserInstance,
  ): Promise<void> {
...
<html>
  ...
  <body>
    ...
    try {
      await instance.nativeOverlayView.webContents.loadURL(
        `data:text/html;charset=UTF-8,${encodeURIComponent(html)}`,
      );
      instance.nativeOverlayReady = true;
      mainLog.info(
        `[browser-pane] native overlay ready id=${instance.id} platform=${liveFxPlatform} corners=${cornerRadii.bottomLeft}/${cornerRadii.bottomRight}`,
      );
      this.updateNativeOverlayState(instance);
    } catch (error) {
      instance.nativeOverlayReady = false;
      mainLog.warn(
        `[browser-pane] native overlay load failed id=${instance.id}: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  private getWindowResizable(window: BrowserWindow): boolean {
    return typeof window.isResizable === "function"
      ? window.isResizable()
      : true;
  }
  ...
  private applyAgentControlLock(
    ...
    if (wantsLock && !instance.lockState.active) {
      instance.lockState.previousResizable = this.getWindowResizable(
        instance.window,
      );
    ...
    if (!wantsLock && instance.lockState.active) {
      this.setWindowResizable(
        instance.window,
        instance.lockState.previousResizable,
      );
      instance.lockState.active = false;
      mainLog.info(
        `[browser-pane] interaction lock released id=${instance.id}`,
      );
    }
  }
  ...
  private prepareForFinalDestroy(instance: BrowserInstance): void {
  ...
  private finalizeDestroyedInstance(
    ...
    this.instanceDestroyedCallback?.(instance.id);
    this.removedCallback?.(instance.id);
    mainLog.info(
      `[browser-pane] Destroyed instance: ${instance.id} (${source})`,
    );
  }
  ...
  private isBrowserEmptyStateUrl(url: string): boolean {
    if (!url) return false;
    return (
      url.includes(`/${BROWSER_EMPTY_STATE_PAGE}`) ||
      url.includes(`\\${BROWSER_EMPTY_STATE_PAGE}`)
    );
  }
  ...
  private async loadEmptyStatePage(instance: BrowserInstance): Promise<void> {
    if (VITE_DEV_SERVER_URL) {
      await instance.pageView.webContents.loadURL(
        `${VITE_DEV_SERVER_URL}/${BROWSER_EMPTY_STATE_PAGE}`,
      );
      return;
    }
  ...
  private async handleDeepLinkUrl(url: string): Promise<void> {
    ...
    try {
      if (!this.windowManager) {
        mainLog.warn(
          "[browser-pane] window manager unavailable for deep-link handling, falling back to shell.openExternal",
        );
      ...
      const resolver = (wcId: number) =>
        this.windowManager?.getClientIdForWindow(wcId);
      ...
      if (!result.success) {
        mainLog.warn(
          `[browser-pane] deep-link handling failed: ${result.error ?? "unknown error"} url=${url}`,
        );
      }
    } catch (error) {
      mainLog.warn(
  ...
  private async maybeHandleEmptyStateLaunch(
    instance: BrowserInstance,
    url: string,
  ): Promise<boolean> {
    ...
    const hash = parsed.hash.startsWith("#")
      ? parsed.hash.slice(1)
      : parsed.hash;
    const launchPayload = hash.startsWith("launch=")
      ? hash.slice("launch=".length)
      : hash;
  ...
  private async loadToolbarPage(instance: BrowserInstance): Promise<void> {
    ...
    for (let attempt = 0; attempt <= TOOLBAR_LOAD_MAX_RETRIES; attempt++) {
      try {
        if (VITE_DEV_SERVER_URL) {
          await instance.toolbarView.webContents.loadURL(
            `${VITE_DEV_SERVER_URL}/browser-toolbar.html?${query}`,
          );
        } else {
        ...
        if (attempt > 0) {
          mainLog.info(
            `[browser-pane] toolbar load recovered id=${instance.id} attempt=${attempt + 1}`,
          );
        }
        return;
      } catch (error) {
  ...
  private async loadToolbarFallback(
    instance: BrowserInstance,
    reason: string,
  ): Promise<void> {
...
<html>
  ...
  registerToolbarIpc(): void {
    ...
    ipcMain.handle(
      ...
      async (_event, instanceId: string, url: string) => {
    ...
    ipcMain.handle(
      TOOLBAR_CHANNELS.MENU_GEOMETRY,
      async (_event, instanceId: string, open: boolean, height?: number) => {
        const inst = findInstance(instanceId);
        if (!inst) return;
    ...
    ipcMain.handle(
      TOOLBAR_CHANNELS.HIDE,
      async (_event, instanceId: string) => {
        const inst = findInstance(instanceId);
        mainLog.info(
          `[browser-pane] toolbar ipc hide requested instanceId=${instanceId} resolved=${inst?.id ?? "none"}`,
        );
    ...
    ipcMain.handle(
      TOOLBAR_CHANNELS.DESTROY,
      async (_event, instanceId: string) => {
        const inst = findInstance(instanceId);
        mainLog.info(
          `[browser-pane] toolbar ipc destroy requested instanceId=${instanceId} resolved=${inst?.id ?? "none"}`,
        );
  ...
  private markToolbarReady(instance: BrowserInstance, reason: string): void {
    ...
    mainLog.info(
      `[browser-pane] toolbar ready id=${instance.id} reason=${reason}`,
    );
  ...
  setAgentControl(
    ...
    for (const instance of this.instances.values()) {
      if (instance.boundSessionId === sessionId) {
        ...
        mainLog.info(
          `[browser-pane] agent control activated session=${sessionId} label=${label}`,
        );
        return;
      }
    }
  ...
  clearAgentControl(sessionId: string): void {
    for (const instance of this.instances.values()) {
      if (
        ...
        this.emitStateChange(instance);
        mainLog.info(
          `[browser-pane] agent control released session=${sessionId}`,
        );
      }
    }
  }
  ...
  clearAgentControlForInstance(
    ...
    this.emitStateChange(instance);
    mainLog.info(
      `[browser-pane] agent control released instance=${instanceId}${sessionId ? ` session=${sessionId}` : ""}`,
    );
  ...
  private async extractThemeColor(instance: BrowserInstance): Promise<void> {
    ...
    try {
      const color = await instance.pageView.webContents.executeJavaScript(
        `(${THEME_COLOR_EXTRACTOR_FN})()`,
      );
  ...
  private registerPopupWindow(
    parentInstance: BrowserInstance,
    ...
    const initialUrl =
      sourceUrl || popupWindow.webContents.getURL?.() || "about:blank";
    mainLog.info(
      `[browser-pane] popup created parent=${parentInstance.id} popupWebContentsId=${popupWcId} url=${initialUrl}`,
    );

    popupWindow.webContents.on("did-navigate", (_event, urlFromEvent) => {
      const popupUrl =
        typeof popupWindow.webContents.getURL === "function"
          ? popupWindow.webContents.getURL()
          : urlFromEvent || initialUrl;
      mainLog.info(
        `[browser-pane] popup did-navigate parent=${parentInstance.id} popupWebContentsId=${popupWcId} url=${popupUrl}`,
      );
    });

    popupWindow.webContents.on(
      "did-redirect-navigation",
      (_event, popupUrl, isInPlace, isMainFrame) => {
        mainLog.info(
  ...
  private unregisterPopupWindow(
    popupWindow: BrowserWindow,
    reason: "closed" | "parent_destroy" | "reparented",
  ): void {
    const popupWcId = popupWindow.webContents.id;
    ...
    mainLog.info(
      `[browser-pane] popup closed parent=${parentId} popupWebContentsId=${popupWcId} reason=${reason}`,
    );
  }

  private closePopupsForParent(
    parentId: string,
    reason: "parent_destroy",
  ): void {
    const popups = this.popupWindowsByParentInstanceId.get(parentId);
  ...
  private pushNetworkLog(
    ...
    if (instance.networkLogs.length > MAX_NETWORK_LOG_ENTRIES) {
      instance.networkLogs.splice(
        0,
        instance.networkLogs.length - MAX_NETWORK_LOG_ENTRIES,
      );
    }
  }
  ...
  private pushDownloadLog(
    ...
    if (instance.downloads.length > MAX_DOWNLOAD_LOG_ENTRIES) {
      instance.downloads.splice(
        0,
        instance.downloads.length - MAX_DOWNLOAD_LOG_ENTRIES,
      );
    }
  }
        ...
        (
          ...
          if (!allowed) {
            this.logPermissionDecision(
              "request",
              permission,
              details?.requestingOrigin ?? "unknown",
            );
          }
          callback(allowed);
        },
  ...
  private setupWindowListeners(instance: BrowserInstance): void {
    ...
    instance.window.on("close", (event) => {
      ...
      const interceptToHide =
        !explicitDestroy && instance.keepAliveOnWindowClose;
      mainLog.info(
    ...
    toolbarWc.on("did-finish-load", () => {
      const loadedUrl =
        typeof toolbarWc.getURL === "function" ? toolbarWc.getURL() : "";
      if (!this.isToolbarUiDocumentUrl(loadedUrl)) {
        mainLog.info(
          `[browser-pane] toolbar did-finish-load ignored id=${instance.id} url=${loadedUrl || "unknown"}`,
        );
        this.pushToolbarState(instance);
        return;
      }
    ...
    toolbarWc.on(
      ...
      (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
        if (!isMainFrame) return;
        mainLog.warn(
          `[browser-pane] toolbar did-fail-load id=${instance.id} code=${errorCode} url=${validatedURL} error=${errorDescription}`,
        );
      },
    );
    ...
    pageWc.on("did-navigate", (_event, urlFromEvent) => {
      const url =
        typeof pageWc.getURL === "function"
          ? pageWc.getURL()
          : urlFromEvent || instance.currentUrl;
      ...
      mainLog.info(
        `[browser-pane] did-navigate id=${instance.id} from=${previousUrl} to=${instance.currentUrl}`,
      );
      instance.canGoBack = pageWc.canGoBack();
      instance.canGoForward = pageWc.canGoForward();
    ...
    pageWc.on(
      ...
      (_event, url, isInPlace, isMainFrame) => {
        if (!isMainFrame) return;
        mainLog.info(
          `[browser-pane] did-redirect-navigation id=${instance.id} url=${url} inPlace=${isInPlace}`,
        );
      },
    );
    ...
    pageWc.on(
      ...
      (_event, errorCode, errorDescription, validatedURL) => {
        mainLog.warn(
          `[browser-pane] did-fail-load id=${instance.id} code=${errorCode} url=${validatedURL} error=${errorDescription}`,
        );
      },
    );
    ...
    pageWc.on("console-message", (_event, level, message) => {
      ...
      const mappedLevel: BrowserConsoleEntry["level"] =
        level >= 3
          ? "error"
          : level === 2
            ? "warn"
      ...
      if (instance.consoleLogs.length > MAX_CONSOLE_LOG_ENTRIES) {
        instance.consoleLogs.splice(
          0,
          instance.consoleLogs.length - MAX_CONSOLE_LOG_ENTRIES,
        );
      }

      if (level >= 2) {
        mainLog.warn(
          `[browser-pane] console id=${instance.id} level=${level}: ${message}`,
        );
      }
    });
    ...
    pageWc.on("did-create-window", (popupWindow, details) => {
      const popupUrl =
        details?.url || popupWindow.webContents.getURL?.() || "about:blank";
    ...
    pageWc.setWindowOpenHandler((details) => {
      ...
      try {
        ...
        mainLog.warn(
          `[browser-pane] window-open denied id=${instance.id} reason=invalid_url url=${details.url}`,
        );
        return { action: "deny" };
      }
